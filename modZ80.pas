...
{
' /*******************************************************************************
'   modZ80.bas within DelphiSpec.dpr
'
...
type
  ...
  TOutByteProc=Procedure(Port:integer;Byte:integer);
...
procedure pushpc;
procedure Z80_Reset;
function Z80_execute(MaxTStates: integer):integer ;

implementation

uses Windows, Forms, Sysutils, new;
...
begin
    bitRes := val And (Not (bit) And $FFFF);
end;
...
function peekb(addr : integer) : integer;
begin
    peekb := Z80_getByte(addr);
end;
...
procedure pokeb(addr : integer; newByte : integer);
begin
   Z80_SetByte(addr,newByte);
end;

procedure pokew(addr : integer; word : integer);
begin
    pokeb(addr, word And $FF);
    pokeb(addr + 1, (word And $FF00) shr 8);
end;
...
function nxtpcw : integer;
  var dummy: integer;
begin
    dummy := peekb(regPC) + (peekb(regPC + 1) * 256);
    nxtpcw := dummy;
{$IFDEF USE_DEBUGGING}
...
begin
    regID := ((byteval * 256) And $FF00) Or (regID And $FF);
end;
...
function peekw(addr : integer) : integer;
begin
      ...
      176:  //LDIR
        begin
          ...
          repeat
            ...
            intRTemp := intRTemp + 2;
            If (TempLocal_tstates >= 0) Then // interruptTriggered
                break;
          until count = 0;
...
function Z80interrupt : integer;

begin

    Result:=0; //tstates for interrupt
    // PERFORM Z80 hardware functions

    if NOT Z80_GetInterrupt then exit;
...
begin
    ...
    local_tstates:=0;
    BreakEmulation:=false;
    // Trigger an interrupt
    repeat
      if application.terminated then break;
      if BreakEmulation and intIFF1=true then
      Begin
       BreakEmulation:=false;
       break;
      End;
      ...
      Case xxx of
        ...
        211:  //OUT (n),A
          begin
            Z80_OutB(nxtpcb, regA);
            local_tstates := local_tstates + 11;
          end;
        219:  //IN A,(n)
          begin
            regA := z80_InB((regA * 256) Or nxtpcb);
            local_tstates := local_tstates + 11;
          end;
        227:  //EX (SP),HL
          begin
      ...
      if Z80_GetInterrupt and intiff1 then break;
      if Z80Steping then
        break;

    until (local_tstates>=MaxTStates);
    local_tstates := local_tstates + Z80interrupt();
    Result:=local_tstates;
end;
