...
begin
  ...
  with Ini, wp.rcNormalPosition do begin
    WriteInteger(form.name+'_'+section, sLeft, Left);
    WriteInteger(form.name+'_'+section, sTop, Top);
    WriteInteger(form.name+'_'+section, sWidth, Right - Left);
    WriteInteger(form.name+'_'+section, sHeight, Bottom - Top);
    WriteString(form.name+'_'+section, sState,
      GetEnumName(TypeInfo(TwindowState), Ord(form.WindowState)));
  end; { With }
  if SaveSplitters then
    Ini.WriteString(form.name+'_'+section, sSplitterPositions,
...
begin
  ...
  if not Assigned(form) then
    raise EParameterCannotBeNil.Create(procname,'form');
  with Ini do begin
    L := ReadInteger(form.name+'_'+section, sLeft, form.Left);
    T := ReadInteger(form.name+'_'+section, sTop, form.Top);
    W := ReadInteger(form.name+'_'+section, sWidth, form.Width);
    H := ReadInteger(form.name+'_'+section, sHeight, form.Height);
    form.WindowState := wsNormal;
    form.SetBounds(L, T, W, H);
    try
      WS :=
        TWindowState(GetEnumValue(TYpeinfo(TWindowState),
        ReadString(form.name+'_'+section, sState, 'wsNormal')));
...
//      end; { case }
  ...
  if RestoreSplitters then
    SetSplitters(form, Ini.ReadString(form.name+'_'+section, sSplitterPositions, ''));
end;
